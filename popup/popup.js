class ShortcutManager {
	constructor() {
		...
		this.blockInTextInputs = true; // Default to blocking in text inputs
	...
	async loadSettings() {
		try {
			...
			this.blockInTextInputs = result.blockInTextInputs !== false; // Default to true

			// Update checkbox state
			document.getElementById("block-text-inputs").checked =
				this.blockInTextInputs;

			// 如果有自定义快捷键，显示自定义选项
			if (this.customShortcut) {
				document.getElementById("custom-shortcut").style.display = "block";
				document.getElementById("custom-input").value =
					this.customShortcut.text;
			}
	...
	async saveSettings() {
		try {
			await chrome.storage.local.set({
				...
				blockInTextInputs: this.blockInTextInputs,
			});
		} catch (error) {
	...
	bindEvents() {
		// 快捷键选择变化
		document
			.getElementById("shortcut-select")
		...
		// Block in text inputs checkbox
		document
			.getElementById("block-text-inputs")
			.addEventListener("change", (e) => {
				this.blockInTextInputs = e.target.checked;
				this.saveSettings();
			});
	}
	...
	recordCustomShortcut(event) {
		...
		if (event.ctrlKey || event.metaKey)
			modifiers.push(this.isMac ? "Command" : "Ctrl");
			...
			if (tabs[0]) {
				...
				if (response && response.status === "ok") {
					if (this.currentShortcut === "custom" && this.customShortcut) {
						this.setStatus(
							"available",
							`自定义快捷键可用: ${this.customShortcut.text}`,
						);
					} else if (this.currentShortcut === "custom") {
...
const translations = {
	en: {
		...
		"block-text-inputs": "Disable shortcut in text inputs",
		"block-text-help": "Prevents search activation when typing in text fields",
	},
	zh: {
		...
		"prev-result": "上一个结果",
		"close-search": "关闭搜索",
		"block-text-inputs": "在文本输入框中禁用快捷键",
		"block-text-help": "防止在文本输入框中输入时激活搜索",
	},
};
