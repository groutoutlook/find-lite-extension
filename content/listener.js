FindLite.listener = (function () {
	...
	let blockInTextInputs = true; // Default to blocking in text inputs

	// 创建防抖的搜索函数
	let debouncedSearch = null;

	// Load blocking settings on initialization
	self.loadSettings = async function () {
		try {
			const result = await chrome.storage.local.get(["blockInTextInputs"]);
			blockInTextInputs = result.blockInTextInputs !== false;
		} catch (error) {
			console.warn("Find Lite: Failed to load settings", error);
		}
	};

	// Listen for settings changes
	self.setupSettingsListener = function () {
	...
	self.isTextInput = function (element) {
	...
	self.registerAllListeners = function () {
		...
		FindLite.panel.element.caseSensitiveButton.onclick =
			self.caseSensitiveClickListener;
		FindLite.panel.element.wholeWordButton.onclick =
			self.wholeWordClickListener;
	...
	self.keydownListener = async function (event) {
		// Check if blocking is enabled and we're in a text input
		if (blockInTextInputs && self.isTextInput(event.target)) {
	...
	self.activateSearch = async function () {
		...
		if (!debouncedSearch) {
			const isRegex = FindLite.panel.isRegex();
			const delay = FindLite.searchOptimizer.getDebounceDelay(isRegex);
			debouncedSearch = FindLite.searchOptimizer.debounce(
				self.performSearch,
				delay,
			);
		}
	...
	self.performSearch = async function (event) {
		...
		try {
			...
			ranges = await FindLite.searchOptimizer.executeTimeLimitedSearch(() => {
				return FindLite.finder.findInBody(
					inputText,
					isCaseSensitive,
					isWholeWord,
					isRegex,
			...
			currentIndex = FindLite.page.determineInitialRangeIndex(
				ranges,
				selectRange,
			);
			// 根据搜索结果渲染显示内容
