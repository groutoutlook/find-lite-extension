FindLite.page = (function () {
	...
	self.determineInitialRangeIndex = function (ranges, selectedRange) {
		...
		if (selectedRange) {
			for (let i = 0; i < ranges.length; i++) {
				const isStartSame =
					ranges[i].compareBoundaryPoints(
						Range.START_TO_START,
						selectedRange,
					) === 0;
				const isEndSame =
					ranges[i].compareBoundaryPoints(Range.END_TO_END, selectedRange) ===
					0;
				const isParentSame =
					ranges[i].commonAncestorContainer ===
	...
	self.scrollToRange = function (range) {
		...
		const isInView =
			rangeRect.top >= 0 && rangeRect.bottom <= window.innerHeight;
		const isInScrollNode =
			rangeRect.top >= scrollNodeRect.top &&
	...
	self.scrollRangeToScrollNode = function (range, scrollNode) {
		...
		if (rangeRect.top < scrollNodeRect.top) {
			scrollY =
				rangeRect.top - scrollNodeRect.top - scrollNode.clientHeight / 2;
		}
		if (rangeRect.bottom > scrollNodeRect.top + scrollNode.clientHeight) {
			scrollY =
				rangeRect.bottom -
				(scrollNodeRect.top + scrollNode.clientHeight) +
				scrollNode.clientHeight / 2;
		}
		if (rangeRect.left < scrollNodeRect.left) {
			scrollX =
				rangeRect.left - scrollNodeRect.left - scrollNode.clientWidth / 2;
		}
		if (rangeRect.right > scrollNodeRect.left + scrollNode.clientWidth) {
			scrollX =
				rangeRect.right -
				(scrollNodeRect.left + scrollNode.clientWidth) +
	...
	self.highlightFocused = function (range) {
		// 高亮
		CSS.highlights.set(
			FindLite.panel.element.currentFocusHighlightName,
			new Highlight(range),
		);
		// 更新定位线位置
